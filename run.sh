...
# Make the email of the user the name of the Docker Network for the services

if [[ -z "$1" || "$1" != "--network-name" || -z "$2" ]]; then
    echo "Usage: $0 --network-name <network-name>"
    exit 1
fi

network_name="$2"

MY_NETWORK=$network_name
...
export PWD=$(pwd)
export MY_NETWORK
$COMPOSE_COMMAND up --no-recreate $COMPOSE_FLAGS